...
func InitDB() *gorm.DB {
	// 读取conf配置文件的数据库配置信息
	dbConfig, err := ini.Load("task/conf/app.init")
	if err != nil {
	...
	path := strings.Join([]string{userName, ":", pwd, "@tcp(", ip, ":", port, ")/", dbName, "?charset=utf8&parseTime=true"}, "")
	// 打开数据库
	...
	Db.Set("gorm:table_options", "ENGINE=InnoDB").AutoMigrate(&entity.Task{})
	return Db
}
